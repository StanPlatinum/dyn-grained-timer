...
unsigned int rdtsc_slope = 0;  /* The number is used to change the slope of the RDTSC return value in hvm rdtsc intercept. */
...
int comportlogptr = 0;	/* the cursor */

#define PRINT_INTERVAL 0x100
static void print_comportflow_log(void) 
{
    int i;
    //printk("\n%s\n", __FUNCTION__);
    /* here for measuring, we only output dbg_data */
...
static void add_comportflow_log4char(char dbg_message) 
{
	...
	print_comportflow_log();	/* once buffer is overflowing, output */
}
...
/* for EPTP-list test*/
#define EPTP_LIST_SIZE 0x200	/* the hardware has 512 items */
...
#define MAX_DOMU_NUM 8
struct pair4guestvm_t {
    int dom_id;
    int n;
}pair4guestvm[MAX_DOMU_NUM];	/* default to be 8. this max value can be modified in field 'max_loop' in dom0's grub file */

/* for dummy request */
#define DUMMY_N_FREQ 1000
unsigned int n_changing_interval = 0;	/* n changing frequency */
unsigned int dummy_n_last = 0;
...
int id_exist(int curr_domain_id)
{
	...
	if (pair4guestvm[i].dom_id == curr_domain_id) return 1;
    return 0;
}

/* for tsc randomization */
...
void hvm_rdtsc_intercept(struct cpu_user_regs *regs)
{
    ...
    int x = 0;	/* temp variable for storing n */
    unsigned int dummy_n = dummy_n_last;

    uint16_t rand16;	/* random number */
    ...
    if ( !id_exist(curr->domain->domain_id) )
	i++; 
    pair4guestvm[i].dom_id = curr->domain->domain_id;
    pair4guestvm[i].n = number_of_bits;
    /* sort out the max n */
    for (j = 0; j < MAX_DOMU_NUM; j++)
	if (pair4guestvm[i].n > x) x = pair4guestvm[i].n;

    /* randomize the n and send dummy request */
    n_changing_interval++;
    /* change dummy_n when necessary */
    if (n_changing_interval > DUMMY_N_FREQ) {
	if(! rdrand16_step(&rand16))	printk("rdrand doing wrong!\n");
	dummy_n = rand16%16;	/* dummy_n \in [0,15] */
	n_changing_interval = 1;
	dummy_n_last = dummy_n;
    }
    
    if (dummy_n > x)	x = dummy_n;
	...
	printk("rdrand16_step: random number not available\n");
    } else {
	printk("rand16= %u\n", rand16);
    }

    /* do another blurring scheme */
...
static hvm_hypercall_t *const hvm_hypercall64_table[NR_hypercalls] = {
    ...
    HYPERCALL(domctl),
    HYPERCALL(tmem_op),
    HYPERCALL(time_slope),
    HYPERCALL(disable_time_slope),
...
/* EPT-list initializaion, only for DomU, tested but failed */
int do_time_slope(int slope)
{
    struct vcpu *v = current;

    if(hypercall_force)
	printk("The time slope part is running now...\n");
    else {
	hypercall_force = 1;
    }
    printk("Slope is : %d\n", slope);
    ...
    unsigned long r;
    int i;
    unsigned long long eptp_list_entry;
    unsigned long long *new_page4eptp = NULL;
    struct domain *d = v->domain;

    printk("Preparing EPT-list...\n");
    if (cpu_has_vmx_ept && cpu_has_vmx_vmfunc) {
	printk("Setting 'EPTP switching' 1...\n");
	...
	printk("EPT pointer = 0x%lx\n", p);
    }
    /* write eptp-list */
    printk("Now in domain %d\n", d->domain_id);
    if ( paging_mode_hap(d) )
    {
	printk("page mode: hap on\n");
	
	printk("manipulating ept list...\n");
	if ( !d->is_dying && altp2m_active(d) ) {
	    printk("altp2m activated\n");
	    __vmwrite(EPTP_LIST_ADDR, virt_to_maddr(d->arch.altp2m_eptp));
	}
	
	if ( (new_page4eptp = alloc_xenheap_page()) == NULL )
	    return -1;
	__vmwrite(EPTP_LIST_ADDR, virt_to_maddr(new_page4eptp));
	eptp_list_entry = (unsigned long long) p;
	for(i = 0; i < 512; i++) {
	   new_page4eptp[i]  = eptp_list_entry;
	}
	__vmread(EPTP_LIST_ADDR, &r);
	printk("EPTP list address = 0x%lx\n", r);
    }
    /* create my alternative p2m(s) and the corresponding eptp(s) and try to use altp2m */
    printk("Domain %d's altp2m eptp-list addr = %lx\n", d->domain_id, virt_to_maddr(d->arch.altp2m_eptp));
    
    for ( i = 0; i < MAX_SLOPE_OPTION; i++ ) {
	p2m->min_remapped_gfn = INVALID_GFN;
	p2m->max_remapped_gfn = 0;

	my_p2m[i] = d->arch.altp2m_p2m[i];
	my_ept[i] = &my_p2m[i]->ept;
	my_ept[i]->asr = pagetable_get_pfn(p2m_get_pagetable(my_p2m[i]));

	new_page4eptp[i] = ept_get_eptp(my_ept[i]);
	printk("altp2m[%d]'s eptp = %llx\n", i, new_page4eptp[i]);
    }
#endif   
    vmx_vmcs_exit(v);
...
int do_disable_time_slope(void)
{
    hypercall_force = 0;  /* still need some RCUs */
    rdtsc_slope = 0;

    if(hypercall_force == 0 && rdtsc_slope == 0)	
	printk("The time slope part has been disabled!\n");
    /* still free_xenheap_page(v) to free the EPTP-list created by hypercall do_time_slope */
    return 0;
}
...
int do_speed_up(int slope)
{
  
    tsc_offset_constant = CYCLE_PER_SECOND >> slope;
